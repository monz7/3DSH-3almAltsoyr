export const posts = [
  {
    id: 1,
    time: "8 دقائق للقراءة",
    date: "١٥ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1500835556837-99ac94a94552?w=800&h=400&fit=crop",
    categ: "إضاءة",
    desc: "إتقان تصوير الساعة الذهبية: دليل شامل",
    title:
      "تعلم كيفية التقاط صور مذهلة خلال الساعة الذهبية مع نصائح احترافية حول الإضاءة والتكوين.",
    name: "سالم أحمد",
    imgItem:
      "	https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop&crop=face",
    jop: "مصور محترف",
    description:
      "الساعة الذهبية هي أكثر الأوقات سحراً للتصوير الفوتوغرافي. ذلك الوقت القصير بعد شروق الشمس وقبل غروبها حيث يكون الضوء ناعماً ودافئاً وساحراً.",
    sections: [
      {
        id: "section-0",
        title: "لماذا الساعة الذهبية؟",
        content:
          "الضوء خلال هذا الوقت له صفات فريدة: ظلال طويلة ناعمة، ألوان دافئة ذهبية، وتباين منخفض يجعل كل شيء يبدو أجمل. البورتريهات تكتسب توهجاً طبيعياً والمناظر الطبيعية تتحول إلى لوحات فنية.",
      },
      {
        id: "section-1",
        title: "التحضير المسبق",
        content:
          "خطط لجلسة التصوير مسبقاً. استخدم تطبيقات لمعرفة وقت الساعة الذهبية بدقة في موقعك. وصل قبل 30 دقيقة لاختيار أفضل زاوية.",
      },
      {
        id: "section-2",
        title: "إعدادات الكاميرا",
        content:
          "استخدم ISO منخفض للحصول على أقل ضوضاء. فتحة العدسة تعتمد على ما تريد: f/1.8-f/2.8 للبورتريهات أو f/8-f/11 للمناظر الطبيعية.",
      },
      {
        id: "section-3",
        title: "التكوين الفني",
        content:
          "ضع الشمس خلف موضوعك للحصول على تأثير الإضاءة الخلفية الساحر أو استخدمها كمصدر جانبي لإبراز العمق.",
      },
      {
        id: "section-4",
        title: "الخلاصة",
        content:
          "الساعة الذهبية هي هدية للمصورين. استغلها جيداً للحصول على صور لا تُنسى.",
      },
    ],

    tags: ["إضاءة", "الساعة الذهبية", "تصوير خارجي"],

    author: {
      name: "سالم أحمد",
      role: "مصور محترف",
      bio: "مصور محترف شغوف بمشاركة المعرفة والخبرات في عالم التصوير الفوتوغرافي.",
      image:
        "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop&crop=face",
    },

    tableOfContents: [
      { id: "section-0", title: "لماذا الساعة الذهبية؟" },
      { id: "section-1", title: "التحضير المسبق" },
      { id: "section-2", title: "إعدادات الكاميرا" },
      { id: "section-3", title: "التكوين الفني" },
      { id: "section-4", title: "الخلاصة" },
    ],
  },
  {
    id: 2,
    time: "6 دقائق للقراءة",
    date: "١٢ يناير ٢٠٢٦",
    image:
      "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=800&h=400&fit=crop",
    categ: "بورتريه",
    desc: "أسرار تصوير البورتريه: كيف تلتقط روح الشخصية",
    title:
      "اكتشف تقنيات احترافية لتصوير بورتريهات تعبيرية تكشف عن شخصية الموضوع الحقيقية.",
    name: "رامي الخطيب",
    imgItem:
      "	https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&fit=crop&crop=face",
    jop: "مصور محترف",
    description:
      "تصوير البورتريه هو فن التقاط جوهر الإنسان في صورة واحدة. ليس مجرد توثيق الملامح، بل كشف القصة خلف العيون.",
    sections: [
      {
        id: "section-0",
        title: "التواصل مع الموضوع",
        content:
          "قبل أن تمسك الكاميرا، تحدث مع الشخص. اجعله يشعر بالراحة. الابتسامة الحقيقية والنظرة الطبيعية تأتي فقط عندما يثق بك الموضوع.",
      },
      {
        id: "section-1",
        title: "اختيار العدسة المناسبة",
        content:
          "عدسات 85mm و 50mm هي الكلاسيكيات لتصوير البورتريه. توفر ضغطاً مثالياً للملامح وخلفية ضبابية جميلة.",
      },
      {
        id: "section-2",
        title: "الإضاءة الطبيعية",
        content:
          "النافذة الكبيرة هي أفضل صديق لمصور البورتريه. ضع الموضوع بزاوية 45 درجة من النافذة للحصول على إضاءة ثلاثية الأبعاد رائعة.",
      },
      {
        id: "section-3",
        title: "التركيز على العيون",
        content:
          "العيون هي نافذة الروح. تأكد دائماً من أن العيون حادة ومركزة. استخدم نقطة تركيز واحدة على العين الأقرب للكاميرا.",
      },
      {
        id: "section-4",
        title: "الخلفية والتكوين",
        content:
          "اختر خلفية بسيطة لا تشتت الانتباه. استخدم قاعدة الأثلاث لوضع العيون في النقاط القوية.",
      },
      {
        id: "section-5",
        title: "الخلاصة",
        content:
          "البورتريه الناجح يحكي قصة. عندما تجمع بين التقنية والتواصل الإنساني، تخلق صوراً خالدة.",
      },
    ],

    tags: ["إضاءة طبيعية", "تصوير أشخاص", "بورتريه"],

    author: {
      name: "محمد علي",
      role: "مصور بورتريه",
      bio: "مصور محترف شغوف بمشاركة المعرفة والخبرات في عالم التصوير الفوتوغرافي.",
      image:
        "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&fit=crop&crop=face",
    },

    tableOfContents: [
      { id: "section-0", title: "التواصل مع الموضوع" },
      { id: "section-1", title: "اختيار العدسة المناسبة" },
      { id: "section-2", title: "الإضاءة الطبيعية" },
      { id: "section-3", title: "التركيز على العيون" },
      { id: "section-4", title: "الخلفية والتكوين" },
      { id: "section-5", title: "الخلاصة" },
    ],
  },
  {
    id: 3,
    time: "10 دقائق للقراءة",
    date: "١٠ يناير ٢٠٢٦",
    image:
      "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800&h=400&fit=crop",
    categ: "مناظر طبيعية",
    desc: "دليل تصوير المناظر الطبيعية: من المبتدئ إلى المحترف",
    title:
      "استكشف تقنيات تصوير المناظر الطبيعية الخلابة وكيفية التقاط جمال الطبيعة بعدستك.",
    name: "إبراهيم حسن",
    imgItem:
      "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop&crop=face",
    jop: "مصور طبيعة",
     description:
      "تصوير المناظر الطبيعية هو رحلة إلى قلب الطبيعة. إنه فن يتطلب الصبر والتخطيط والعين الفنية لرؤية الجمال في كل مكان.",
    sections: [
      {
        id: "section-0",
        title: "المعدات الأساسية",
        content:
          "حامل ثلاثي قوي ضروري للحصول على صور حادة. عدسة واسعة الزاوية (16-35mm) مثالية لالتقاط المشاهد الواسعة. فلاتر ND و Polarizer ستفتح لك آفاقاً إبداعية جديدة.",
      },
      {
        id: "section-1",
        title: "توقيت التصوير",
        content:"الساعة الزرقاء والساعة الذهبية هي أفضل الأوقات. الضوء يكون ناعماً والألوان غنية. لا تخف من التصوير في الطقس الدرامي - الغيوم والعواصف تضيف شخصية للصورة."
      },
      {
        id: "section-2",
        title: "التكوين الفني",
        content:"ابحث عن عناصر المقدمة المثيرة للاهتمام. صخرة، زهرة، أو مسار يقود العين نحو الخلفية. استخدم خطوط التوجيه لخلق عمق في الصورة.",
      },
      {
        id: "section-3",
        title: "إعدادات الكاميرا",
        content:"فتحة f/8-f/16 للحصول على حدة من المقدمة للخلفية. ISO منخفض قدر الإمكان. استخدم وضع Live View والتكبير للتركيز اليدوي الدقيق.",
      },
      {
        id: "section-4",
        title: "المعالجة اللاحقة",
        content:"صور بصيغة RAW لأقصى مرونة في التعديل. تعديل الإضاءة والألوان بلطف يبرز جمال المشهد دون إفراط.",
      },
      {
        id: "section-5",
        title: "الخلاصة",
        content:"تصوير المناظر الطبيعية يعلمك التأمل والصبر. كل صورة هي ذكرى من رحلة لا تُنسى.",
      },
    ],

    tags: ["مناظر طبيعية", "تصوير خارجي", "طبيعة"],

    author: {
      name: "إبراهيم حسن",
      role: "مصور طبيعة",
      bio: "مصور محترف شغوف بمشاركة المعرفة والخبرات في عالم التصوير الفوتوغرافي.",
      image:
        "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&fit=crop&crop=face",
    },

    tableOfContents: [
      { id: "section-0", title: "المعدات الأساسية" },
      { id: "section-1", title: "توقيت التصوير" },
      { id: "section-2", title: "التكوين الفني" },
      { id: "section-3", title: "إعدادات الكاميرا" },
      { id: "section-4", title: "المعالجة اللاحقة" },
      { id: "section-5", title: "الخلاصة" },
    ],
  },
  {
    id: 4,
    time: "7 دقائق للقراءة",
    date: "٨ يناير ٢٠٢٦",
    image:
      "https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "أساسيات إعدادات الكاميرا: مثلث التعريض الضوئي",
    title:
      "افهم العلاقة بين فتحة العدسة وسرعة الغالق وحساسية ISO للتحكم الكامل في صورك.",
    name: "داود خالد",
    imgItem:
      "https://images.unsplash.com/photo-1560250097-0b93528c311a?w=100&h=100&fit=crop&crop=face",
    jop: "مدرب تصوير",
  },
  {
    id: 5,
    time: "9 دقائق للقراءة",
    date: "٥ يناير ٢٠٢٦",
    image:
      "https://images.unsplash.com/photo-1452587925148-ce544e77e70d?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "قواعد التكوين الفوتوغرافي: كيف تجعل صورك أكثر جاذبية",
    title:
      "تعلم قواعد التكوين الأساسية التي يستخدمها المصورون المحترفون لإنشاء صور مؤثرة بصرياً.",
    name: "ليث محمود",
    imgItem:
      "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=100&h=100&fit=crop&crop=face",
    jop: "فنان بصري",
  },
  {
    id: 6,
    time: "8 دقائق للقراءة",
    date: "٣ يناير ٢٠٢٦",
    image:
      "https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=800&h=400&fit=crop",
    categ: "معدات",
    desc: "تصوير الهاتف المحمول: كيف تلتقط صوراً احترافية بهاتفك",
    title:
      "اكتشف كيف تحول هاتفك الذكي إلى أداة تصوير قوية مع هذه النصائح والتقنيات.",
    name: "جمال عبدالله",
    imgItem:
      "https://images.unsplash.com/photo-1463453091185-61582044d556?w=100&h=100&fit=crop&crop=face",
    jop: "مصور ومراجع تقني",
  },
  {
    id: 7,
    time: "11 دقائق للقراءة",
    date: "١ يناير ٢٠٢٦",
    image:
      "https://images.unsplash.com/photo-1519681393784-d120267933ba?w=800&h=400&fit=crop",
    categ: "إضاءة",
    desc: "تصوير الليل والنجوم: دليلك لالتقاط سماء الليل",
    title:
      "تعلم كيفية تصوير النجوم ودرب التبانة والمناظر الليلية الساحرة مع هذه التقنيات المتقدمة.",
    name: "خالد الفيصل",
    imgItem:
      "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=100&h=100&fit=crop&crop=face",
    jop: "مصور فلكي",
  },
  {
    id: 8,
    time: "7 دقائق للقراءة",
    date: "٢٨ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1517732306149-e8f829eb588a?w=800&h=400&fit=crop",
    categ: "بورتريه",
    desc: "تصوير الشارع: فن التقاط الحياة اليومية",
    title:
      "اكتشف أسرار تصوير الشارع وكيفية توثيق اللحظات العفوية في الحياة اليومية.",
    name: "نادر سعيد",
    imgItem:
      "https://images.unsplash.com/photo-1566492031773-4f4e44671857?w=100&h=100&fit=crop&crop=face",
    jop: "مصور شوارع",
  },
  {
    id: 9,
    time: "8 دقائق للقراءة",
    date: "٢٥ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1476224203421-9ac39bcb3327?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "تصوير الطعام: كيف تجعل أطباقك تبدو شهية",
    title:
      "تعلم تقنيات تصوير الطعام الاحترافية لإنشاء صور تثير الشهية وتجذب العيون.",
    name: "هاني الشمري",
    imgItem:
      "https://images.unsplash.com/photo-1552058544-f2b08422138a?w=100&h=100&fit=crop&crop=face",
    jop: "مصور طعام",
  },
  {
    id: 10,
    time: "10 دقائق للقراءة",
    date: "٢٢ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1549366021-9f761d450615?w=800&h=400&fit=crop",
    categ: "مناظر طبيعية",
    desc: "تصوير الحياة البرية: كيف تلتقط عجائب الطبيعة",
    title:
      "دليل شامل لتصوير الحيوانات في بيئتها الطبيعية مع نصائح للمعدات والتقنيات.",
    name: "عمر الراشد",
    imgItem:
      "https://images.unsplash.com/photo-1507591064344-4c6ce005b128?w=100&h=100&fit=crop&crop=face",
    jop: "مصور حياة برية",
  },
  {
    id: 11,
    time: "9 دقائق للقراءة",
    date: "٢٠ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1533134486753-c833f0ed4866?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "التصوير بالأبيض والأسود: فن الضوء والظل",
    title: "اكتشف جمال التصوير أحادي اللون وكيفية إنشاء صور قوية بدون ألوان.",
    name: "فارس العلي",
    imgItem:
      "https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?w=100&h=100&fit=crop&crop=face",
    jop: "فنان فوتوغرافي",
  },
  {
    id: 12,
    time: "12 دقائق للقراءة",
    date: "١٨ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1572044162444-ad60f128bdea?w=800&h=400&fit=crop",
    categ: "معدات",
    desc: "أساسيات تعديل الصور في Lightroom",
    title:
      "تعلم كيفية استخدام Adobe Lightroom لتحسين صورك وإنشاء أسلوب بصري مميز.",
    name: "سامي الحربي",
    imgItem:
      "https://images.unsplash.com/photo-1568602471122-7832951cc4c5?w=100&h=100&fit=crop&crop=face",
    jop: "خبير تعديل صور",
  },
  {
    id: 13,
    time: "10 دقائق للقراءة",
    date: "١٥ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1550159930-40066082a4fc?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "عالم التصوير الماكرو: اكتشف التفاصيل الخفية",
    title:
      "انغمس في عالم التصوير المقرب واكتشف جمال التفاصيل الصغيرة التي تفوتنا بالعين المجردة.",
    name: "رامي الخطيب",
    imgItem:
      "https://images.unsplash.com/photo-1548372290-8d01b6c8e78c?w=100&h=100&fit=crop&crop=face",
    jop: "مصور ماكرو",
  },
  {
    id: 14,
    time: "8 دقائق للقراءة",
    date: "١٢ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1501785888041-af3ef285b470?w=800&h=400&fit=crop",
    categ: "إضاءة",
    desc: "التعريض الطويل: كيف تصور الحركة والزمن",
    title:
      "تعلم تقنيات التعريض الطويل لإنشاء صور إبداعية تظهر الحركة بطريقة فنية ساحرة.",
    name: "باسم المصري",
    imgItem:
      "https://images.unsplash.com/photo-1583195764036-6dc248ac07d9?w=100&h=100&fit=crop&crop=face",
    jop: "مصور فني",
  },
  {
    id: 15,
    time: "11 دقائق للقراءة",
    date: "١٠ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1519741497674-611481863552?w=800&h=400&fit=crop",
    categ: "بورتريه",
    desc: "تصوير حفلات الزفاف: دليل المصور المحترف",
    title:
      "تعلم أساسيات تصوير حفلات الزفاف من التحضير إلى تسليم الصور النهائية.",
    name: "منصور الزهراني",
    imgItem:
      "https://images.unsplash.com/photo-1564564321837-a57b7070ac4f?w=100&h=100&fit=crop&crop=face",
    jop: "مصور زفاف",
  },
  {
    id: 16,
    time: "9 دقائق للقراءة",
    date: "٨ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1473968512647-3e447244af8f?w=800&h=400&fit=crop",
    categ: "معدات",
    desc: "التصوير بالدرون: منظور جديد للعالم",
    title:
      "اكتشف عالم التصوير الجوي وتعلم أساسيات استخدام الدرون لالتقاط صور من زوايا فريدة.",
    name: "فيصل الدوسري",
    imgItem:
      "https://images.unsplash.com/photo-1618077360395-f3068be8e001?w=100&h=100&fit=crop&crop=face",
    jop: "مصور جوي",
  },
  {
    id: 17,
    time: "8 دقائق للقراءة",
    date: "٥ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "تصوير المنتجات: أساسيات التصوير التجاري",
    title:
      "تعلم كيفية تصوير المنتجات بشكل احترافي لمتجرك الإلكتروني أو عملائك التجاريين.",
    name: "لؤي الصالح",
    imgItem:
      "https://images.unsplash.com/photo-1633332755192-727a05c4013d?w=100&h=100&fit=crop&crop=face",
    jop: "مصور تجاري",
  },
  {
    id: 18,
    time: "9 دقائق للقراءة",
    date: "٢ ديسمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1486325212027-8081e485255e?w=800&h=400&fit=crop",
    categ: "مناظر طبيعية",
    desc: "تصوير العمارة: كيف تلتقط روح المباني",
    title:
      "اكتشف تقنيات تصوير المباني والهندسة المعمارية بطريقة فنية تبرز جمالها وتفاصيلها.",
    name: "طارق النعيمي",
    imgItem:
      "https://images.unsplash.com/photo-1607990281513-2c110a25bd8c?w=100&h=100&fit=crop&crop=face",
    jop: "مصور معماري",
  },
  {
    id: 19,
    time: "10 دقائق للقراءة",
    date: "٢٨ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1461896836934-ffe607ba8211?w=800&h=400&fit=crop",
    categ: "بورتريه",
    desc: "تصوير الرياضة والحركة: تجميد اللحظة الحاسمة",
    title:
      "تعلم تقنيات تصوير الأحداث الرياضية والحركة السريعة بوضوح ودقة احترافية.",
    name: "أحمد الشهري",
    imgItem:
      "https://images.unsplash.com/photo-1580518324671-c2f0833a3af3?w=100&h=100&fit=crop&crop=face",
    jop: "مصور رياضي",
  },
  {
    id: 20,
    time: "8 دقائق للقراءة",
    date: "٢٥ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1542038784456-1ea8e935640e?w=800&h=400&fit=crop",
    categ: "إضاءة",
    desc: "أساسيات التصوير بالفلاش: تحكم كامل في الإضاءة",
    title:
      "افهم كيفية استخدام الفلاش الخارجي لإنشاء إضاءة احترافية في أي موقف.",
    name: "ماجد القحطاني",
    imgItem:
      "https://images.unsplash.com/photo-1543610892-0b1f7e6d8ac1?w=100&h=100&fit=crop&crop=face",
    jop: "مصور استوديو",
  },
  {
    id: 21,
    time: "9 دقائق للقراءة",
    date: "٢٢ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=800&h=400&fit=crop",
    categ: "مناظر طبيعية",
    desc: "تصوير السفر: كيف توثق رحلاتك بصور لا تُنسى",
    title: "نصائح عملية لتصوير السفر تساعدك على التقاط جوهر كل مكان تزوره.",
    name: "ياسر العتيبي",
    imgItem:
      "https://images.unsplash.com/photo-1590086782957-93c06ef21604?w=100&h=100&fit=crop&crop=face",
    jop: "مصور رحالة",
  },
  {
    id: 22,
    time: "7 دقائق للقراءة",
    date: "١٨ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1502691876148-a84978e59af8?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "نظرية الألوان في التصوير: كيف تستخدم الألوان بذكاء",
    title: "افهم كيف تؤثر الألوان على مشاعر المشاهد وكيف تستخدمها لتعزيز صورك.",
    name: "دحام الحسيني",
    imgItem:
      "https://images.unsplash.com/photo-1504257432389-52343af06ae3?w=100&h=100&fit=crop&crop=face",
    jop: "فنان بصري",
  },
  {
    id: 23,
    time: "10 دقائق للقراءة",
    date: "١٥ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1519689680058-324335c77eba?w=800&h=400&fit=crop",
    categ: "بورتريه",
    desc: "تصوير المواليد: فن التقاط البراءة",
    title: "تعلم تقنيات تصوير الأطفال حديثي الولادة بأمان واحترافية.",
    name: "نايف المطيري",
    imgItem:
      "https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?w=100&h=100&fit=crop&crop=face",
    jop: "مصور مواليد",
    link: "/blog/newborn-baby-photography",
  },
  {
    id: 24,
    time: "8 دقائق للقراءة",
    date: "١٢ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "تصوير العقارات: كيف تجعل المنزل يبيع نفسه",
    title: "تعلم تقنيات تصوير العقارات التي تجعل المنازل تبدو أفضل ما يمكن.",
    name: "عبدالله الغامدي",
    imgItem:
      "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=100&h=100&fit=crop&crop=face",
    jop: "مصور عقارات",
  },
  {
    id: 25,
    time: "6 دقائق للقراءة",
    date: "٨ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&h=400&fit=crop",
    categ: "معدات",
    desc: "RAW مقابل JPEG: متى تستخدم كلاً منهما",
    title: "افهم الفرق بين صيغتي الصور الأكثر شيوعاً واختر الأنسب لاحتياجاتك.",
    name: "كريم الفهد",
    imgItem:
      "https://images.unsplash.com/photo-1534030347209-467a5b0ad3e6?w=100&h=100&fit=crop&crop=face",
    jop: "خبير تقني",
    link: "/blog/raw-vs-jpeg-explained",
  },
  {
    id: 26,
    time: "7 دقائق للقراءة",
    date: "٥ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1554080353-a576cf803bda?w=800&h=400&fit=crop",
    categ: "بورتريه",
    desc: "تصوير البورتريه الذاتي: كن موضوعك الخاص",
    title: "تعلم كيف تصور نفسك بشكل احترافي وإبداعي دون الحاجة لمساعد.",
    name: "سلطان الراجحي",
    imgItem:
      "https://images.unsplash.com/photo-1557862921-37829c790f19?w=100&h=100&fit=crop&crop=face",
    jop: "فنان تصوير",
    link: "/blog/self-portrait-photography",
  },
  {
    id: 27,
    time: "9 دقائق للقراءة",
    date: "٢ نوفمبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1617005082133-548c4dd27f35?w=800&h=400&fit=crop",
    categ: "معدات",
    desc: "دليل العدسات للمبتدئين: كيف تختار عدستك الأولى",
    title: "افهم أنواع العدسات المختلفة واختر الأنسب لأسلوب تصويرك واحتياجاتك.",
    name: "فهد السبيعي",
    imgItem:
      "https://images.unsplash.com/photo-1531891437562-4301cf35b7e4?w=100&h=100&fit=crop&crop=face",
    jop: "مراجع معدات",
    link: "/blog/lens-guide-beginners",
  },
  {
    id: 28,
    time: "6 دقائق للقراءة",
    date: "٢٨ أكتوبر ٢٠٢٥",
    image:
      "https://images.unsplash.com/photo-1494438639946-1ebd1d20bf85?w=800&h=400&fit=crop",
    categ: "تقنيات",
    desc: "التصوير البسيط (Minimalist): قوة الفراغ",
    title: "اكتشف جمال البساطة في التصوير وكيف تخلق صوراً قوية بعناصر قليلة.",
    name: "راشد الجاسر",
    imgItem:
      "https://images.unsplash.com/photo-1545167622-3a6ac756afa4?w=100&h=100&fit=crop&crop=face",
    jop: "فنان بصري",
    link: "/blog/minimalist-photography",
  },
  
];
